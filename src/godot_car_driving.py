...
def get_current_frame():
    while True:
            raw_data = recv_all(conn, TOTAL_EXPECTED)
            if raw_data: break
    
    data = np.frombuffer(raw_data, dtype=np.uint8).reshape((HEIGHT, WIDTH))
    img = torch.from_numpy(data)
    # print("image received !!")
    return transform(img)


def send_response(speed, steer):
    response = struct.pack("<ff", speed, steer)
...
def setup_server():
    ...
    server.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1) # to reduce latency
...
def loop():
    ...
    with torch.no_grad():
        while True:
            ...
            mask_to_show = mask[0, 0].cpu().numpy()
            cv2.imshow("AI Road Mask", mask_to_show)
            cv2.waitKey(1)
            
            mask = torch.sigmoid(mask)
            speed, steer = calculate(mask)
            send_response(speed, steer)
